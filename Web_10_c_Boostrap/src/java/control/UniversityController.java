...
public class UniversityController extends HttpServlet {
    ...
    private String validateUniversity(UniversityDTO u, boolean isUpdate) {
        ...
        if (u.getShortName() == null || u.getShortName().trim().isEmpty()) {
            error.append("Chưa nhập Short Name <br/>");
        }
        if (u.getDescription() == null || u.getDescription().trim().isEmpty()) {
            error.append("Chưa nhập Description <br/>");
        }
        if (u.getAddress() == null || u.getAddress().trim().isEmpty()) {
            error.append("Chưa nhập Address <br/>");
        }
        if (u.getCity() == null || u.getCity().trim().isEmpty()) {
            error.append("Chưa nhập City <br/>");
        }
        if (u.getRegion() == null || u.getRegion().trim().isEmpty()) {
            error.append("Chưa nhập Region <br/>");
        }
        if (u.getType() == null || u.getType().trim().isEmpty()) {
            error.append("Chưa nhập Type <br/>");
        }
        if (u.getFoundedYear() < 0) {
            error.append("Năm thành lập phải >= 0 <br/>");
        }
        if (u.getTotalStudents() < 0) {
            error.append("Tổng sinh viên phải >= 0 <br/>");
        }
        if (u.getTotalFaculties() < 0) {
            error.append("Tổng giảng viên phải >= 0 <br/>");
        }
